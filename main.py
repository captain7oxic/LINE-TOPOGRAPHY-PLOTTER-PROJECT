...
if __name__ == "__main__":
    ...
    dir = 'C:\\Users\\Vatsal\\Desktop\\codes\\Line-topography-plotter-project\\'
    map = tiff.imread(dir + 'worldmap.tiff')
    ...
    for y_value in y_values:
        ...
        if smoothen:
            ...
            z_int[[remove_values[int(n_points * i / n_int_points)]
                   for i in range(n_int_points)]] = np.nan
        else:
        ...
        colors = [cmap(0.15+0.85*value/highest_value)
                  if not np.isnan(value) else cmap(0) for value in z_int]
        widths = [0.1 + 0.2*value / highest_value for value in z_int]

        lines.plot_line_2d(
            ax, x_int, y_int, z_int, z_fraction=0.003, linewidths=widths, linecolors=colors)
